        ...
        <div class="sm:px-6 col-span-full">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 text-center">
                    <h1 class="text-center text-xl font-semibold">You are updating tag ({{ $tag->name }})</h1>
                    <form action="{{ route('tags.update', $tag->id) }}" method="POST">
                        ...
                        <div class="mx-auto md:py-0 mt-5 flex flex-col gap-5">
                            ...
                            @enderror
                            <input type="text" placeholder="Enter Tag Name" class="input input-bordered w-full" name="name" value="{{ old('name', $tag->name) }}" />
                            <div class="flex flex-col md:flex-row gap-2 md:gap-4 justify-center items-center">
                                ...
                                <button type="submit" class="btn btn-active btn-primary md:grow">Save Tag</button>
                            </div>
                        </div>
                    </form>
                </div>
